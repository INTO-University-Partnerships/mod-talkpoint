...
require_once __DIR__ . '/../src/mod_talkpoint_restore_controller.php';

/**
 * @see http://docs.moodle.org/dev/Restore_2.0_for_developers
 */
class mod_talkpoint_restore_test extends advanced_testcase {
    ...
    /**
     * @var mod_talkpoint_restore_controller
     */
    protected $_cut;
    ...
    protected function setUp() {
        ...
        $this->_cut = new mod_talkpoint_restore_controller(
            $folder,
            $this->_courseid,
    ...
    public function test_execute_plan_restores_module() {
        ...
        $this->assertEquals(1, $DB->count_records('talkpoint'));
        $data = (array)$DB->get_record('talkpoint', array(), '*', MUST_EXIST);
        $this->assertSame('Talkpoint 001', $data['name']);
        ...
        $this->assertEquals(0, $DB->count_records('talkpoint_talkpoint'));
        $this->assertEquals(0, $DB->count_records('talkpoint_comment'));
    }
